...
public class InputValidator { //검증
    ...
    private static void list_check(String[] list) { //연속된 구분자 있는지 확인
    ...
    private static void only_num_check(String[] list) { //숫자만 입력되었는지 검증
        if (list.length == 1 && Integer.parseInt(list[0]) >0){
            throw new IllegalArgumentException("숫자만 입력되었습니다.");
        }
    }

    private static void sep_check(String s) { //분리된 문자열이 원하는 형태로 잘 변환될 수 있는지 확인
        if (s.isEmpty()){
            return;
        }
        try {
